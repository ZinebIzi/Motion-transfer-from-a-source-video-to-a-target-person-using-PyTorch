...
class GenVanillaNN():
    ...
    def __init__(self, videoSke, loadFromFile=False, optSkeOrImage=1):
        ...
        # Debug: print the value of optSkeOrImage
        print(f"optSkeOrImage: {optSkeOrImage}")
...
class GenNNSkeImToImage(nn.Module):
    ...
    def __init__(self, input_dim=26):  # Default input dimension set to 26
        ...
        self.model = nn.Sequential(
            ...
            # Output Layer
            nn.Linear(16384, 3 * 64 * 64),
            nn.Tanh()  # Scale output to [-1, 1]
        )

    def forward(self, z):
        # Ensure the input is flattened to match the expected shape
        if len(z.shape) > 2:  # Flatten if z is not already flat
            z = z.view(z.size(0), -1)
        img = self.model(z)
