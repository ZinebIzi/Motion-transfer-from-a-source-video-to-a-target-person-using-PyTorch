...
def _xyzto012(c):
    if c in 'xyz':
        return ord(c) - ord('x') #ord(c) gives the ASCII value of character c
    else:
...
def _args2tuple(funcname, args):
    narg = len(args)
    if narg == 0: #No Arguments
        data = 3*(0,) # data = (0, 0, 0)
    elif narg == 1: #Single Argument
        data = args[0]
        if len(data) != 3: # the function assumes it’s a sequence (e.g., a list or tuple) containing three elements.
            raise TypeError('Vec3.%s() takes sequence with 3 elements '
                            '(%d given),\n\t   when 1 argument is given' %
                            (funcname, len(data)))
    elif narg == 3: # Three Separate Arguments 
        data = args
    else:
