...
#mp_pose = mp.solutions.pose
#pose is the specific solution used for detecting human body poses.
#Calling Pose() initializes the pose detector, which performs the following:
    #Loads the pre-trained model that detects and tracks human poses.
    #Configures default parameters for pose detection and tracking, such as detection confidence thresholds and whether to use specific pose landmarks 
...
class Skeleton:
    ...
    def __array__(self, dtype=None, reduced=False):
        ...
        if reduced:
            #np.vstack() function vertically stacks the selected joints (in this case, the 13 joints) into a 2D NumPy array.
            return np.vstack( self.ske[self.reduce_indice] ).astype(float)[:, :2]
        else:
            #  Stacks all 33 joints from self.ske vertically into a 2D NumPy array.
    ...
    def fromImage(self, image):     
        ...
        results = mp_pose_detector.process(image) #This object contains the detected pose landmarks, along with other metadata.
        if results.pose_landmarks is None:
            return False
        if results.pose_landmarks:
            for index, landmark in enumerate(results.pose_landmarks.landmark): 
                #enumerate() provides both the index (position of the landmark in the list) and the landmark
                #results.pose_landmarks.landmark is a list of 33 landmarks                   
    ...
    def draw(self, image):
        ...
        image.flags.writeable = True #This ensures that the image can be modified
        #image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        height,width,_ = image.shape
        for i in range(33):
            x, y = int(self.ske[i].x * width), int(self.ske[i].y * height)
            #draws a circle at the position (x, y) with a radius of 3 pixels
    ...
    @staticmethod
    def neck(ske,w,h): #calculates the average position of the neck based on the coordinates of the left shoulder (ls) and right shoulder (rs)
    ...
    @staticmethod
    def pelvis(ske,w,h): #calculates the pelvis position by averaging the positions of the left hip (lh) and right hip (rh)
